...
final defaultEvents = ListBuilder<UserEvent>([
  UserEvent((e) => e
      ...
      ..submission_deadline = DateTime.now().add(const Duration(hours: 4))
      ..submission_start = DateTime.now().subtract(const Duration(hours: 1))
  ),

  UserEvent((e) => e
    ..event_id = "demo_event"
    ..type = EventType.other.name
    ..name = "Test Event"
    ..accountability_method = AccountabilityMethod.self_signed.name
    ..time = DateTime.now().add(const Duration(hours: 3))
    ..status = UserStatus.unsigned.name
    ..submission_deadline = DateTime.now().add(const Duration(hours: 4))
    ..submission_start = DateTime.now().subtract(const Duration(hours: 1))
...
final InitialData initial = InitialData((i) => i
    ...
    ..event_excusals = ListBuilder()
    ..recurring_excusals = ListBuilder()
);
