...
class Excusals extends StatelessWidget {
  ...
  static successMessage(BuildContext context, String verb) {
    ScaffoldMessenger.of(context)
        .showSnackBar(SnackBar(content: Text("Successfully ${verb}d excusal")));
  }

  static failureMessage(BuildContext context, String verb) {
    ScaffoldMessenger.of(context)
        .showSnackBar(SnackBar(content: Text("Failed to $verb excusal")));
  }

  static openExcusalDialog(BuildContext context, EventExcusal? excusal,
          void Function(EventExcusal) onSubmit) =>
      showDialog(
        context: context,
  ...
  static openRecurringDialog(BuildContext context, RecurringExcusal? recurring,
          void Function(RecurringExcusal) onSubmit) =>
      showDialog(
        context: context,
        builder: (context) => Dialog(
          insetPadding: const EdgeInsets.all(20),
              ...
              children: [
                ExcusalList<EventExcusal>(
                  excusals: model.content.excusals
                      .where((e) => model.content.events
                          .firstWhere((ev) => ev.event_id == e.event_id)
                          .submission_deadline
                          .isAfter(DateTime.now()))
                      .toList(),
                  add: (excusal) => model
                      .dispatch(ExcusalAction.createExcusal(excusal: excusal)),
                  converter: (excusal) => EventExcusalDescriptor(
                    excusal: excusal,
                    onEdit: () => openExcusalDialog(
                        context,
                        excusal,
                        (excusal) => model.dispatch(ExcusalAction.createExcusal(
                              excusal: excusal,
                            ))),
                    onDelete: () => model.dispatch(
                        ExcusalAction.deleteExcusal(excusal: excusal)),
                  ),
                ),
                ElevatedButton(
            ...
            PageWidget(
              ...
              children: [
                ExcusalList<RecurringExcusal>(
                  ...
                  add: (excusal) =>
                      model.dispatch(ExcusalAction.createRecurring(
                    recurring: excusal,
                  )),
                  converter: (excusal) => RecurringExcusalDescriptor(
                    ...
                    onDelete: () =>
                        model.dispatch(ExcusalAction.deleteRecurring(
                      recurring: excusal,
                    )),
                  ),
                ),
                ElevatedButton(
                  onPressed: () => openRecurringDialog(
                      ...
                      null,
                      (excusal) => model.dispatch(
                          ExcusalAction.createRecurring(recurring: excusal))),
                  child: const Padding(
                    padding: EdgeInsets.all(8),
                    child: Icon(
                      Icons.add,
