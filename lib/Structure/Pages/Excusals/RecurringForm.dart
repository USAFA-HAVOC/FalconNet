...
class RecurringFormState extends State<RecurringForm> {
  ...
  static const List<String> dayLiterals = ["M", "Tu", "W", "Th", "F", "Sa", "Su"];
  ...
  void initState() {
    ...
    events = Map.fromEntries(EventType.values.map((v) => MapEntry(v.name, widget.excusal?.event_types.contains(v.name) ?? false)));
          ...
          child: ListView(
            ...
            children: [
              ...
              GridView.count(
                  ...
                  children: events.entries.map((e) => Column(
                    ...
                    children: [
                      ...
                      Checkbox(
                          value: e.value,
                          onChanged: (state) => setState(() => events[e.key] = state!)
                      )
                    ],
                  )).toList(),
              ),
              ...
              DropdownButtonFormField<String?>(
                  ...
                  items: RecurringExcusalType.values.map((e) => DropdownMenuItem<String?>(
                      value: e.name,
                      child: Text(RecurringExcusalTypeNames.parse(e.name).display())
                  )).toList() + const [DropdownMenuItem<String?>(
                      value: null,
                      child: Text("Select")
                  )],
              ...
              if (time == RecurringExcusalType.days_of_week.name) GridView.count(
                crossAxisCount: 4,
                crossAxisSpacing: 10,
                mainAxisSpacing: 10,
                shrinkWrap: true,
                primary: false,
                padding: const EdgeInsets.symmetric(vertical: 10),
                children: days.asMap().entries.map((e) => Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    ...
                    Checkbox(
                        value: e.value,
                        onChanged: (state) => setState(() => days[e.key] = state!)
                    )
                  ],
                )).toList(),
              ),
