...
class SEEventState extends State<SEEvent> {
  ...
  List<Widget> buildGradeeBars() => gradees.map(
    (gradee) => Padding(
  ...
  void submit(ScaffoldMessengerState messenger) async {
    ...
    if (parsedScore == null) {
      ...
      messenger.showSnackBar(const SnackBar(content: Text("Gradee List Cannot Be Empty")));
    }
    else {
      setState(() {
        descriptionError = null;
      ...
      try {
        ...
        messenger.showSnackBar(const SnackBar(content: Text("Failed to Submitted Grade")));
      }
    }
  }
