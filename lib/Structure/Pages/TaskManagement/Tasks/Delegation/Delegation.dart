...
class DelegationState extends State<Delegation> {
  ...
  void initState() {
    ...
    connection = Future.delayed(Duration(milliseconds: 250), () => DelegationData(
        delegates: [
          Delegate(name: "Rylie Anderson", id: "ra", roles: [TimedRole(role: Role(type: RoleType.cwoc), start: DateTime(2022, 11, 24), end: DateTime(2023, 1, 23))]),
          Delegate(name: "Ethan Chapman", id: "ec", roles: []),
          Delegate(name: "Enrique Oti", id: "eo", roles: [TimedRole(role: Role(type: RoleType.sdo, level: RoleLevel.delegation), start: DateTime(2022, 11, 24), end: DateTime(2023, 1, 21))]),
          Delegate(name: "David Petzold", id: "dp", roles: []),
        ],

        owner: [
          TimedRole(role: Role(type: RoleType.cwoc, level: RoleLevel.staff), start: DateTime(2023, 1, 1), end: DateTime(2023, 1, 22)),
          TimedRole(role: Role(type: RoleType.jdo, level: RoleLevel.delegation), start: DateTime(2022, 11, 23), end: DateTime(2023, 1, 22)),
        ]
    ));
  }
            ...
            builder: (context, snapshot) {
              if (snapshot.data != null) {
                return ListView.builder(
                    shrinkWrap: true,
                    primary: false,
                    itemCount: snapshot.data!.delegates.length,
                    itemBuilder: (context, index) => DelegateBar(
                        delegate: snapshot.data!.delegates[index],
